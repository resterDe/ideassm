...
//设置类别列表
    ...
    p32.tpl = '<li>'+
        ...
        '<h2>%s</h2>'+
        '<p style="color: red;">%s 元</p>'+
        '<p class="ui-li-aside"><a href="#" onclick="removeCar2(%s);" style="font-size: 20px;text-decoration:none;">删除</a></p>'+
        '</li>';
    p32.colums = ["img","gname","price","id"];
    $("#cars2").data("property",JSON.stringify(p32));
...
function refreshGood(title,type,uid){
    ...
    ajaxCallback("listJ",{typeid:stype,sid:focushop.id,uid:uid,statecn:statecn,table:"good"},function(data){
...
function listGood(sid){
    var statecn = "待交易";
    ajaxCallback("listJ",{sid:sid,statecn:statecn,table:"good"},function(data){
        goodlist = data;
        $("#goods").refreshShowListView(data);
    });
}
...
function payBill() {
    ...
    //if (userinfo.money && userinfo.money * 1 > focusobj.total * 1) {
        ...
        focusobj.table="bill";
        ajaxCallback("saveJ", focusobj, function () {
            //if(data){
...
function tijiaouser(){
    ...
    ajaxCallback("saveJ",bill,function(){
        ...
        showTipTimer("订单提交成功!",function(){
            billDetail(bill);
            ajaxCallback("updateSqlJ",{sql:"update wct_good set statecn='已交易' where id="+bill.gids},function(data){
    ...
    var sql="update wct_good set statecn='已交易' where id in ("+bill.gids+")";
...
function addToCar2(){
    ...
    var str = localStorage[gouwuche2];
    var list = [];
    if(str){
        list = JSON.parse(str);
    }
    list.push(focusobj);
    localStorage[gouwuche2] = JSON.stringify(list);
    showLoader("已经添加到收藏!",true);
}

function showCar2(){
    changePage("carspage2");
    carlist2();
}
...
function carlist2(){
    ...
    $("#cars2").refreshShowListView(list);
}

function removeCar2(id){
    var str = localStorage[gouwuche2];
    var list = [];
    var newlist = [];
    if(str){
        ...
        localStorage[gouwuche2] = JSON.stringify(newlist);
        $("#cars2").refreshShowListView(newlist);
    }
}
...
function refreshServerChatList(){
    ...
    var sql= "select * from wct_message u where 1=1  and (u.uid="+queryobj.uid+" or u.fid="+queryobj.uid+") and (u.uid="+queryobj.fid+" or u.fid="+queryobj.fid+")";
    if(_chattype==2){
        sql="select * from wct_message u where 1=1  and u.qid="+queryobj.fid;
    }
    ajaxCallback("listSqlJ",{sql:sql},function(data){
...
function checkIsMyFriend(cb){
    ajaxCallback("findJ",{id:userinfo.id,table:"user"},function(fuser){
        userinfo = fuser;
        var fids = fuser.fids;
        var flag = false;
        if(fids){
